...
class PredictionModel:
  ...
  def _ctmc(self, particle, timestamp_secs, only_connected=True):
    ...
    diffs_norm = np.dot(self.node_diffs2D[particle.node, pos_connected_nodes], 
                        self.node_diffs2D[particle.node, pos_connected_nodes].transpose()).diagonal()
  ...
  def predict(self, particle, timestamp_secs, only_connected=False):
    if self.pred_type == PredictionModel.CTMC:
      ...
      return self._identity(node)  # TODO: Which node is this?
